...
[ $? != 0 ] && exit 1
tar xzvf MailScanner-5.1.2-2.rhel.tar.gz
[ $? != 0 ] && exit 1
yum -y install MailScanner-5.1.2-2/MailScanner-5.1.2-2.noarch.rpm
[ $? != 0 ] && exit 1
cd $GITPATH/rpmbuild/SPECS
[ $? != 0 ] && exit 1
...
echo 'echo "First time login: root/eFaPr0j3ct"' >> /etc/rc.d/rc.local
