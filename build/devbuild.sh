...
[ $? != 0 ] && exit 1
yum -y install $GITPATH/rpmbuild/RPMS/x86_64/MailWatch-1.2.12-1.eFa.el7.x86_64.rpm
[ $? != 0 ] && exit 1
...
[ $? != 0 ] && exit 1
tar xzvf MailScanner-5.1.3-1.rhel.tar.gz
[ $? != 0 ] && exit 1
yum -y install MailScanner-5.1.3-1/MailScanner-5.1.3-1.noarch.rpm
[ $? != 0 ] && exit 1
cd $GITPATH/rpmbuild/SPECS
[ $? != 0 ] && exit 1
