...
cat > %{buildroot}%{_unitdir}/mailscanner_relay.service << 'EOF'
[Unit]
Description=Postfix relay service for MailWatch
SourcePath=/usr/bin/mailwatch/tools/Postfix_relay
After=network-online.target remote-fs.target rsyslog.service postfix.service mailscanner.service
...
GuessMainPID=no
RemainAfterExit=no
ExecStart=/usr/bin/php -q /usr/bin/mailwatch/tools/Postfix_relay/mailwatch_mailscanner_relay.php
...
%attr(0644, root, root) %{_unitdir}/mailscanner_relay.service
...
%changelog
* Tue Nov 30 2021 Shawn Iverson <siverson@ena.com> - 1.2.18-5
- Build for ENA
